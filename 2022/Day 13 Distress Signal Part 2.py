...
for i in range(len(packets)):
    j = 0
    while j < len(packets_sorted):
        if compare_packets(packets[i], packets_sorted[j]) == 1:
            break
        j += 1
    packets_sorted.insert(j, packets[i])
    
divider_2 = packets_sorted.index([[2]]) + 1
divider_6 = packets_sorted.index([[6]]) + 1
